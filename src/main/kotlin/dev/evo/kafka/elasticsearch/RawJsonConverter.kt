...
class RawJsonConverter : BaseConverter() {
    ...
    override fun fromConnectData(topic: String, schema: Schema?, value: Any?): ByteArray? {
        ...
        return (value as String).toByteArray(Charsets.UTF_8)
    }
    ...
    override fun toConnectData(topic: String, headers: Headers?, value: ByteArray?): SchemaAndValue {
        ...
        val bulkAction = try {
            ...
            when (meta) {
                is BulkMeta.Index -> BulkAction.Index(meta, RawJsonSource(parseSource(value)))
                is BulkMeta.Delete -> BulkAction.Delete(meta)
                is BulkMeta.Update -> BulkAction.Update(meta, RawJsonSource(parseSource(value)))
                is BulkMeta.Create -> BulkAction.Create(meta, RawJsonSource(parseSource(value)))
            }
        } catch (e: Throwable) {
    ...
    private fun parseSource(value: ByteArray?): String {
